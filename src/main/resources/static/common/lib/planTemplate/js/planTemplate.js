...
//            alert('111');
        ...
        render: function(pos) {
            ...
            if (this.settings.type == 'holiday') {
                ...
                for (var i = 0; i < len; i++) {
                    //TODO: holidy不再显示两个日期，只显示一个日期
                    //把日期里面的时分秒过滤掉 - yusijiayf1
                    var dateStr = this.date[i].split(',')[0].substring(0,10);
                    html.push('<div class="planTemplateView-bar data-index="' + i + '"></div>',
                            '<div class="planTemplateView-label">' + dateStr + '</div>');
...
//                              '</div>');
                ...
                // 每次加入内容之前先初重置innerHTML里面的内容，否则会重复累加 -yusijiayf1
                this.$context.context.innerHTML = '<div class="planTemplateView-panel"><div class="planTemplateView-scale"></div><div class="planTemplateView-time"></div></div>';

                this.$context.append($(html.join('')));

                //内容被重置后，监听器也要重新添加 - yusijiayf1
                this.hover();
            ...
            if (this.hasType) {
                ...
                for (var i = 0; i < pos.length; i++) {
                    ...
                    for (var j = 0; j < pos[i].length; j++) {
                        ...
                        //TODO:holiday类型暂时用黄色表示
                        if(this.settings.type == 'holiday'){
                            html.push('<div class="planTemplateView-slider orange" style="width:' + (pos[i][j][1] - pos[i][j][0]) + 'px; left:'
                                + pos[i][j][0] + 'px;height:'+ (this.settings.barHeight-6) +  'px;"></div>');
//                            html.push('<div class="planTemplateView-slider yellow" style="width:' + (pos[i][j][1] - pos[i][j][0]) + 'px; left:' + pos[i][j][0] +'px;"></div>');
                        }else{
